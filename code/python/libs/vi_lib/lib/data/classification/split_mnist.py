...
from lib.data.classification.permuted_mnist import Task
...
class SplitMnistGenerator():
    ...
    def get_N(self):
        return self.X_train.shape[0] / len(self.sets_0)
    ...
    def next_task(self):
        if self.cur_iter >= self.max_iter:
            ...
            a = self.Y_train[train_0_id] - 2 * self.cur_iter
            b = self.Y_train[train_1_id] - 2 * self.cur_iter
            next_y_train = np.concatenate((a, b))
            ...
            # next_y_test = np.vstack((self.Y_test[test_0_id], self.Y_test[test_1_id]))
            a = self.Y_test[test_0_id] - 2 * self.cur_iter
            b = self.Y_test[test_1_id] - 2 * self.cur_iter
            next_y_test = np.concatenate((a, b))
...
class SplitMnistTasks:
    def __init__(self, no_tasks, batch_size, use_cuda):
        self.tasks = []
        self.use_cuda = use_cuda

        generator = SplitMnistGenerator()
